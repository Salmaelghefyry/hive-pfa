...
import { useAuth } from '@/contexts/AuthContext'; // To access the register function
...
const AddUserForm: React.FC<AddUserFormProps> = ({ onClose, onUserAdded }) => {
  const [formData, setFormData] = useState({
    ...
    password: '',
    confirmPassword: '',
    department: '', // Should be a Department type, but using string for initial state
  });
  ...
  const handleChange = (e: React.ChangeEvent<HTMLInputElement | HTMLSelectElement>) => {
    ...
    if (errors[id]) {
      setErrors((prev) => { delete prev[id]; return { ...prev }; });
    }
  };
  ...
  const validate = () => {
    ...
    // Password validation - reuse logic from Register.tsx if possible
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // Ensure department is of the correct type before calling register
    const departmentValue = formData.department as Department;

    setLoading(true);
    try {
      const success = await register(formData.email, formData.password, formData.username, departmentValue);
      if (success) {
        toast({
          title: "User Added!",
        ...
        onUserAdded(); // Call the parent callback to close modal and refresh list
         ...
         // register function already shows a toast on failure, but we can add more specific handling here if needed
         // setErrors({ ...errors, general: "Failed to register user." });
      }
    } catch (error: any) {
      console.error('Add User error:', error);
       // The register function in AuthContext already handles and shows toast for API errors.
       // If you need specific form field errors, you'd parse error.message here.
       let errorMsg = "Failed to create account";
       if (error && error.message) {
         errorMsg = error.message;
       }
       setErrors({ ...errors, general: errorMsg }); // Display a general error if register didn't handle it
    }
    setLoading(false);
  };
         ...
         <div className="grid grid-cols-4 items-center gap-4">
          <Label htmlFor="department" className="text-right">Department</Label>
           ...
           >
            ...
            {errors.department && <p className="col-span-4 text-right text-red-500 text-xs mt-1">{errors.department}</p>}
        </div>
      </form>
      <DialogFooter>
